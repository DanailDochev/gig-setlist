...
const RIDDLE = 'Забавление, при което ентусиасти пеят под акомпанимента на известни песни, само инструментала. Обикновено има сцена с микрофон и екран, на който се показва текста на песента заедно с музиката.';
...
function Grain({ o = 0.18 }) {
  ...
  useEffect(() => {
    ...
    const ctx = cv.getContext('2d'); let t;
    const draw = () => {
    ...
    draw(); return () => clearTimeout(t);
  }, []);
  return <canvas ref={ref} style={{
    position:'absolute', inset:0, width:'100%', height:'100%',
    pointerEvents:'none', mixBlendMode:'overlay', opacity:o, imageRendering:'pixelated',
  }}/>;
}
...
function Screen({ children, scroll = false }) {
  return (
    <div style={{
      position:'absolute', inset:0, background:THEME.bg, overflow: scroll ? 'auto' : 'hidden',
...
function PinScreen({ onSuccess }) {
    ...
    setDigits(prev => {
      ...
      if (next.length === 4) {
        if (next.join('') === SECRET_PIN) {
          ...
          setTimeout(() => {
            setShake(true);
            setTimeout(() => { setShake(false); setDigits([]); }, 480);
          }, 60);
        }
      }
      return next;
  ...
  const del = () => { if (!ok) setDigits(p => p.slice(0, -1)); };

  return (
    <Screen>
      <div style={{
        position:'absolute', inset:0, display:'flex', flexDirection:'column',
        alignItems:'center', justifyContent:'center', gap:38, padding:'0 24px',
      }}>
        {/* label */}
        <div style={{ textAlign:'center' }}>
          <div style={{
            fontFamily:'var(--mono)', fontSize:9, letterSpacing:6,
            color:THEME.accent, textTransform:'uppercase', marginBottom:12,
          }}>Финален акт · 3 / 3</div>
          <div style={{
            fontFamily:'var(--display)', fontSize:38, fontWeight:300,
            fontStyle:'italic', color:THEME.fg, lineHeight:1,
          }}>Секретен код</div>
        </div>

        {/* dot indicators */}
        <div style={{ display:'flex', gap:16, animation: shake ? 'ul-pinShake 0.48s ease' : 'none' }}>
          {[0,1,2,3].map(i => {
            ...
            return (
              <div key={i} style={{
                ...
                background: filled ? (ok ? THEME.accent : THEME.fg) : 'transparent',
        ...
        {/* numpad */}
        <div style={{
          display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, width:224,
        }}>
          {[1,2,3,4,5,6,7,8,9,null,0,'⌫'].map((k, i) => (
            <button key={i}
              onClick={() => k === '⌫' ? del() : k != null ? press(String(k)) : null}
              style={{
                height:58, borderRadius:4,
                border:`1px solid ${THEME.hair}`,
                background: k == null ? 'transparent' : THEME.alt,
                color: k === '⌫' ? THEME.muted : THEME.fg,
                fontFamily:'var(--mono)', fontSize: k === '⌫' ? 18 : 22,
                cursor: k == null ? 'default' : 'pointer',
                outline:'none', WebkitTapHighlightColor:'transparent',
                transition:'transform 0.08s, background 0.1s',
              }}
              onPointerDown={e => { if (k != null) e.currentTarget.style.transform = 'scale(0.91)'; }}
...
function WaxSeal({ cracking }) {
  ...
  const cracks = [
    [90,6,87,90],[150,36,90,90],[162,98,90,90],
    [126,168,90,90],[26,150,90,90],[14,76,90,90],
  ];
  ...
  return (
    <svg width="190" height="190" viewBox="0 0 180 180" fill="none">
      ...
      <circle cx="90" cy="90" r="63" fill="none" stroke="#f0ece41a" strokeWidth="1.2"/>
      <circle cx="90" cy="90" r="56" fill="none" stroke="#f0ece40e" strokeWidth="0.7"/>

      {/* monogram */}
      <text x="90" y="111" textAnchor="middle"
        fontFamily="Georgia, serif" fontSize="70"
        fontStyle="italic" fontWeight="300"
        fill="#f0ece4" opacity="0.9">К</text>

      {/* crack lines */}
      {cracking && cracks.map(([x1,y1,x2,y2], i) => (
        <line key={i}
          x1={x1} y1={y1} x2={x2} y2={y2}
          stroke="#f0ece4" strokeWidth={1.4 - i * 0.1}
          opacity={0.75 - i * 0.07}
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            ...
            animation: phase === 'cracking' ? 'ul-sealShake 0.18s ease infinite' : 'none',
          }}
        >
        ...
        }}>Натисни, за да разбиеш печата</div>
      </div>
    </Screen>
  );
}
  ...
  const guess = (letter) => {
    ...
    if (WORD.includes(letter)) {
      ...
      if (WORD.every((l, i) => SHOWN.has(i) || next.has(l))) {
        setTimeout(() => { setSolved(true); setTimeout(onSolve, 1600); }, 500);
      }
    } else {
      setWiggle(letter);
    ...
    <Screen scroll>
      <div style={{
        ...
        <div>
          <div style={{
            fontFamily:'var(--mono)', fontSize:9, letterSpacing:6,
            color:THEME.accent, textTransform:'uppercase', marginBottom:11,
          }}>Финалната загадка</div>
          <div style={{
            fontFamily:'var(--serif-text)', fontStyle:'italic',
            ...
            return (
              <div key={i} style={{ textAlign:'center', width:32 }}>
                <div style={{
                  ...
                  animation: !preset && visible && !isFlash ? 'ul-letterPop 0.38s ease' : 'none',
                }}>{letter}</div>
                <div style={{
                  height:2, borderRadius:1,
                  background: visible
                    ? (preset ? THEME.muted : THEME.accent)
                    : `${THEME.hair}cc`,
...
function UnlockFlow({ onUnlock }) {
  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%' }}>
      ...
      {step === 'riddle' && <RiddleScreen onSolve={() => { setStep('done'); onUnlock?.(); }} />}
    </div>
  );
}
