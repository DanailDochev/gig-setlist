...
function ProgramRow({ item, index, theme, isNow, past, onOpen, unlocked, onSetNow }) {
      ...
      <div style={{
        ...
        <div style={{ flex: 1, position: 'relative', zIndex: 1, paddingLeft: 12 }}>
          ...
          <div style={{
            ...
            fontStyle: 'italic', marginTop: 2, letterSpacing: '-0.01em', color: theme.accent
          }}>
            {item.title}
          </div>
          <div style={{
            fontFamily: 'var(--serif-text)', fontSize: 13,
            color: theme.muted, fontStyle: 'italic', marginTop: 4
          }}>
            {item.note}
          </div>
        </div>
  ...
  return (
    <button onClick={onOpen} style={{
      ...
      <span
        onClick={e => { e.stopPropagation(); onSetNow(item.id); }}
...
export function ProgramView({ theme, program, nowId, onOpen, unlocked, onSetNow }) {
  const nowIndex = nowId != null ? program.findIndex(item => item.id === nowId) : -1;
  const nowItem = nowIndex >= 0 ? program[nowIndex] : null;

  const songCount = program.filter(i => i.type === 'song' && !i.secret).length;
    ...
    <div data-program-scroll style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span>{songCount} {songCount === 1 ? 'песен' : 'песни'}</span>
          {breakCount > 0 && <><span>·</span><span>{breakCount} {breakCount === 1 ? 'пауза' : 'паузи'}</span></>}
          {hasSecret && <><span>·</span><span>1 изненада</span></>}
          {unlocked && <><span>·</span><span style={{ color: theme.accent }}>+5 тайни</span></>}
        </div>
      </div>
